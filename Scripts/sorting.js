...
let arrsizer=document.querySelector('#sizer');   //whenever the sizer slider changes geenratebars function will be invoked
arrsizer.addEventListener('input', function(){
    generateBars(parseInt(arrsizer.value))});
generateBars();   //by default we will call generateBars

let time=100;
let sortspeed=document.querySelector('#speed');
sortspeed.addEventListener('input',function(){
    time=850-parseInt(sortspeed.value);
});

function generateBars(noBar=60){
document.getElementById("bar").innerHTML=''; //i have make sured that previous bars will be empty
let bars=[]; 
